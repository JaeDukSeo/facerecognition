...
$("#arrowLeft").click(function() {
  let itemWidth =
    parseInt($("#mask-list ul li").css("width")) +
...
async function detectFaces() {
  ...
  await model
    .estimateFaces(inputElement, false, flipHorizontal)
    .then(predictions => {
  ...
  if (predictions.length > 0) {
    for (let x = 0; x < predictions.length; x++) {
      ...
      switch (maskType) {
        case "full":
          maskCoordinate = {
            top: dots[overheadIndex].top,
            left: dots[leftCheekIndex].left
          };
      ...
      maskSizeAdjustmentWidth = parseFloat(
        selectedMask.attr("data-scale-width")
      );
      maskSizeAdjustmentHeight = parseFloat(
        selectedMask.attr("data-scale-height")
      );
      ...
      maskTop =
        maskCoordinate.top -
        (maskHeight * (maskSizeAdjustmentHeight - 1)) / 2 -
        maskHeight * maskSizeAdjustmentTop;
      maskLeft =
        maskCoordinate.left -
        (maskWidth * (maskSizeAdjustmentWidth - 1)) / 2 +
        maskWidth * maskSizeAdjustmentLeft;
...
function getCoordinate(x, y) {
  if (isVideo) {
    if (
      webcam.webcamList.length == 1 ||
      window.innerWidth / window.innerHeight >=
        webcamElement.width / webcamElement.height
    ) {
